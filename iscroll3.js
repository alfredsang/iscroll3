...
function iScroll3 (el, options) {
	...
	that.options = {
		...
		isFullScreen:true,//add by alfred.sang@androtimes.com

		onScrollXPageFliped: function () {} //add by alfred.sang
	};
	...
	//add by alfred
	that.onScrollXPageFliped = that.options.onScrollXPageFliped;
	delete that.options.onScrollXPageFliped;
...
iScroll3.prototype = {
	...
	/**
	 * add by alfred.sang@androtimes.com
	 *
	 * iScroll3只有onScrollEnd事件的回调函数，并不支持已翻页的回调函数
	 * 
	 * _isPageFliped 是在iScroll3只有onScrollEnd中通过registerPageFliped(currentXPage,this.x)方法，返回的标记
	 */
	currentXPage:1,
	_isPageFliped:false, 
...
//			that.dist+= Math.abs(leftDelta) + Math.abs(topDelta);
	...
	/*判断是否翻页*/
	//add by alfred.sang
	verifyIsXPageFliped: function(){
		var that = this;
		var scrollEndPageNum =  Math.abs(Math.floor(that.x/that.wrapper.offsetWidth))+1;
		//alert('scroll but no flip');
		//alert(scrollEndPageNum+'--'+that.currentXPage);
		if( scrollEndPageNum == that.currentXPage){
			//alert('scroll but no flip');
			that._isPageFliped=false;
		}else{
			//alert('scroll and fliped');
			that._isPageFliped=true;
			that.currentXPage=scrollEndPageNum;
		}

		//如果_isPageFliped为true,回调onScrollXPageFliped函数
		if(that._isPageFliped){
			that.onScrollXPageFliped();
		}
	},
